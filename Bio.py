            ...
            if re.fullmatch(r"лаб[ау]{,2}", toxt, flags=re.ASCII): # регулярка
                ...
                for i in lab_lines: # цикл for по всем строкам в тексте лабы
                    if "🧪 Готовых патогенов:" in i:
                        ...
                        sms += f"<emoji document_id=5237746191231362257>🧐</emoji> готовые паты:{s}\n"
                    if "⏱ Новый патоген:" in i:
                        s = i.replace("⏱ Новый патоген:", "")
                        sms += f"<emoji document_id=5240026024296586587>⏰</emoji> новый пат:{s}\n"
                    if "🦠 Заразность:" in i:
                        s = i.replace("🦠 Заразность:", "")
                        sms += f"<emoji document_id=5238167158155912247>🤮</emoji> зз:{s}\n"
                    if "🧬 Био-ресурс:" in i:
                        s = i.replace("🧬 Био-ресурс:", "")
                        sms += f"<emoji document_id=5238231217593130586>🤑</emoji> ресов:{s}\n"
                    if "❗️ Руководитель в состоянии горячки, вызванной болезнью" in i:
                        ...
                        sms += f"<emoji document_id=5238067596519022555>💀</emoji> ты больной блять от {s}\n"
                    if "❗️ Руководитель в состоянии горячки ещё" in i:
                        s = i.replace("❗️ Руководитель в состоянии горячки ещё ", "")
                        sms += f"<emoji document_id=5238067596519022555>💀</emoji> ты больной блять на {s}\n"                        
